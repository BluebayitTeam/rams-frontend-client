...
function CustomPhoneWithCountryCode({
  ...
  const selectedCode = watch(countryName);

  const selectedCountry = countryCodes.find((c) => c.value === selectedCode);

  useEffect(() => {
    const currentValue = getValues(countryName);
    if (!currentValue) {
      const defaultCountry = countryCodes.find((c) => c.label === "Bangladesh");
      if (defaultCountry) {
        setValue(countryName, defaultCountry.value, { shouldValidate: true });
        if (onChange) onChange(defaultCountry);
      }
    }
  }, [countryName, getValues, onChange, setValue]);
      ...
      <Controller
        ...
        render={({ field }) => (
          <Autocomplete
            ...
            sx={{ width: 500 }}
            value={
              countryCodes.find((country) => country.value === field.value) ||
              null
            }
