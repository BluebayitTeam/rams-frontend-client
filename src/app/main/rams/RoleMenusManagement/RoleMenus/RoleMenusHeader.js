...
const RoleMenusHeader = () => {
    ...
    useEffect(() => {
        const alert = localStorage.getItem("roleMenuAlert")

        if (alert === "saveRoleMenu") {
            setAlertOpen(true)
            setAlertMessage("Add Success...")
            localStorage.removeItem("roleMenuAlert")
        }
        if (alert === "updateRoleMenu") {
            setAlertOpen(true)
            setAlertMessage("Update Success...")
            localStorage.removeItem("roleMenuAlert")
        }
        if (alert === "deleteRoleMenu") {
            setAlertOpen(true)
        ...
        <div className="flex flex-1 w-full items-center justify-between">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
                <Button
                    ...
                    color="secondary"
                >
                    <span className="hidden sm:flex">Add New RoleMenu</span>
