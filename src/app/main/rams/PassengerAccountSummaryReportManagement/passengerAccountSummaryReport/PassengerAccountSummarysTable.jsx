...
} from '../passengerAccountSummarysApi';
...
function PassengerAccountSummaryReportsTable(props) {
  ...
  const { watch } = methods;
  const [
    modifiedPassengerAccountSummaryData,
    setModifiedPassengerAccountSummaryData,
  ] = useReportData();
  ...
  const { data: paginatedData, refetch: refetchPassengerAccountSummary } =
    useGetPassengerAccountSummaryReportsQuery(
      {
  ...
  const { data: allData, refetch: refetchAllPassengerAccountSummary } =
    useGetPassengerAccountSummaryAllReportsQuery(
      {
        agent: filterData.agent || '',
      },
    ...
    <div className={classes.headContainer}>
      ...
      <table
        ...
        <tbody ref={componentRef} id='downloadPage'>
          ...
          {modifiedPassengerAccountSummaryData.map(
            (passengerAccountSummary, index) => (
              <SiglePageWithExtraHeading
                ...
                serialNumber={
                  pagination
                    ? page * size -
                      size +
                      index * passengerAccountSummary.data.length +
                      1
                    : passengerAccountSummary.page *
                        passengerAccountSummary.size -
                      passengerAccountSummary.size +
                      1
                }
                setPage={setPage}
