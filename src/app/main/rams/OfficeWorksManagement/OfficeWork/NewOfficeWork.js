...
const OfficeWork = () => {
	...
	useEffect(() => {
		if (fromSearch) {
			axios
				.get(`${OFFICEWORK_BY_PASSENGER_ID}${officeWorkId}`)
				.then(res => {
								...
								render={({ field: { onChange, value, name } }) => (
									<Autocomplete
										...
										onChange={(event, newValue) => {
											if (newValue?.id) {
												axios
													.get(`${OFFICEWORK_BY_PASSENGER_ID}${newValue?.id}`)
													.then(res => {
														console.log('Res', res.data);
														if (res.data.id) {
															reset({
															...
															history.push(
																`/apps/officeWork-management/officeWork/${
															...
															reset({
																...
																finger_status: doneNotDone.find(data => data.default)
																	?.id
															});
														}
													})
