...
import { setMenusSearchText } from '../store/menusSlice';
...
const MenusHeader = () => {
    ...
    useEffect(() => {
        const alert = localStorage.getItem("menuAlert")
    ...
    return (
        <div className="flex flex-1 w-full items-center justify-between">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
                <Button
                    component={Link}
                    to="/apps/menu-management/new"
                    className="whitespace-nowrap"
                    variant="contained"
                    color="secondary"
                >
                    <span className="hidden sm:flex">Add New Menu</span>
