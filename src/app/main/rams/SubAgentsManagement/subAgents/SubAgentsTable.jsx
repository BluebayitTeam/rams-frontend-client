...
function SubAgentsTable(props) {
  ...
  const { data, isLoading, refetch } = useGetSubAgentsQuery({
    ...pageAndSize,
    searchKey,
  });
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const totalData = useSelector(selectFilteredSubAgents(data));
  const subAgents = useSelector(selectFilteredSubAgents(data?.sub_agent));
  ...
  function handleSelectAllClick(event) {
    if (event.target.checked) {
      setSelected(subAgents.map((n) => n.id));
      return;
    }
...
export default withRouter(SubAgentsTable);
