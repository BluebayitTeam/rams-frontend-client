...
function FlightFilterMenu({ inShowAllMode, handleGetFlights, handleGetAllFlights }) {
			...
			<div className="allFieldContainer borderTop mt-4">
							...
							return (
								<DatePicker
									...
									onChange={value => {
										...
										inShowAllMode ? handleGetAllFlights() : handleGetFlights();
									}}
								/>
							);
						}}
				...
				<div className="fieldContainer">
					<FontAwesomeIcon
						className="icon cursor-pointer"
						icon={faCalendarAlt}
						onClick={() => document.getElementById('flightDateAfterEl').click()}
					/>

					<div className="dateLabel" onClick={() => document.getElementById('flightDateAfterEl').click()}>
						Flight From
					</div>
					...
					<Controller
						...
						render={({ field }) => {
							return (
								<DatePicker
									id="flightDateAfterEl"
									className="hidden"
									autoOk
									clearable
									format={'dd/MM/yyyy'}
									maxDate={values.flight_date_before || new Date()}
									value={field.value || ''}
									onChange={value => {
										value
											? field.onChange(moment(new Date(value)).format('YYYY-MM-DD'))
				...
				<div className="fieldContainer">
					<FontAwesomeIcon
						className="icon cursor-pointer"
						icon={faCalendarAlt}
						onClick={() => document.getElementById('flightDateBeforeEl').click()}
					/>

					<div className="dateLabel" onClick={() => document.getElementById('flightDateBeforeEl').click()}>
						Flight To
					</div>
				...
				<div className="fieldContainer">
					...
					<input
						...
						onKeyDown={e => {
							if (e.key === 'Enter') {
								setValue('ticket_no', e.target.value);
								inShowAllMode ? handleGetAllFlights() : handleGetFlights();
								ticketNoEl.current.blur();
								ticketNoEl.current.value = '';
								setReRender(Math.random());
							}
						}}
						onFocus={() => (ticketNoEl.current.value = ticketNoEl.current.value || values.ticket_no || '')}
						className="textField"
						style={{ width: '60px' }}
			...
			<div className="allKeyWrdContainer">
				...
				{values.flight_date_after && (
					<div className="keywordContainer">
						...
						<div>
							<FontAwesomeIcon className="iconWithKeyWord" icon={faCalendarAlt} />
							<p>{moment(new Date(values.flight_date_after)).format('DD-MM-YYYY')}</p>
							<FontAwesomeIcon
								className="closeIconWithKeyWord"
								icon={faTimesCircle}
								onClick={() => {
									setValue('flight_date_after', '');
									inShowAllMode ? handleGetAllFlights() : handleGetFlights();
									setReRender(Math.random());
								}}
							/>
				...
				{values.flight_date_before && (
					<div className="keywordContainer">
						<b>Flight To</b>
						<div>
							<FontAwesomeIcon className="iconWithKeyWord" icon={faCalendarAlt} />
							<p>{moment(new Date(values.flight_date_before)).format('DD-MM-YYYY')}</p>
							<FontAwesomeIcon
								className="closeIconWithKeyWord"
								icon={faTimesCircle}
								onClick={() => {
									setValue('flight_date_before', '');
									inShowAllMode ? handleGetAllFlights() : handleGetFlights();
									setReRender(Math.random());
								}}
							/>
