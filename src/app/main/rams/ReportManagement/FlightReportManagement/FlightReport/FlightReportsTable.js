...
import { getAllFlights, getFlights } from '../store/flightReportSlice';
...
const initialTableColumnsState = [
	...
	{ id: 6, label: 'F.Ent Date', name: 'created_at', show: true, type: 'date' },
	{ id: 7, label: 'T.Agency', name: 'ticket_agency', subName: 'username', show: true },
	{ id: 8, label: 'T.No', name: 'ticket_no', show: true },
	{ id: 9, label: 'Sector Name', name: 'sector_name', show: true },
	{ id: 10, label: 'T.Status', name: 'ticket_status', show: true },
	{ id: 11, label: 'Carrier AirWay', name: 'carrier_air_way', show: true },
	{ id: 12, label: 'Iss.Date', name: 'issue_date', show: true, type: 'date' },
	{ id: 13, label: 'Arvl.Time', name: 'arrival_time', show: true },
	{ id: 14, label: 'F.No', name: 'flight_no', show: true },
	{ id: 15, label: 'Flight Date', name: 'flight_date', show: true, type: 'date' },
	{ id: 16, label: 'F.Time', name: 'flight_time', show: true }
];
...
const FlightReportsTable = () => {
	...
	//get flights
	const handleGetFlights = (pagePram, callBack) => {
		dispatch(getFlights({ values: getValues(), pageAndSize: { page: pagePram || page, size } })).then(res => {
			unstable_batchedUpdates(() => {
				callBack && callBack(res.payload);
				setModifiedFlightData(res.payload?.flights || []);
	...
	//get all flight without pagination
	const handleGetAllFlights = (callBack, callBackAfterStateUpdated) => {
		dispatch(getAllFlights(getValues())).then(res => {
			unstable_batchedUpdates(() => {
				callBack && callBack(res.payload);
				setModifiedFlightData(res.payload?.flights || []);
				...
				//get pagination data
				const { totalPages, totalElements } = getPaginationData(res.payload?.flights, size, page);
				setPage(page || 1);
				setSize(size || 25);
		...
		<>
			...
			<table id="table-to-xls" className="w-full" style={{ minHeight: '270px' }}>
				<div ref={componentRef} id="downloadPage">
					...
					{modifiedFlightData.map(flight => (
						<SinglePage
							classes={classes}
							generalData={generalData}
							reporTitle="Flight Report"
							tableColumns={tableColumns}
							dispatchTableColumns={dispatchTableColumns}
							data={flight}
							serialNumber={flight.page * flight.size - flight.size + 1}
							setPage={setPage}
