...
import { getAllReceipts, getReceipts } from '../store/receiptReportSlice';
...
const ReceiptReportsTable = () => {
	...
	const [modifiedReceiptData, setModifiedReceiptData, setSortBy] = useReportData({ extraRowCount: 1 });
	...
	//get receipts
	const handleGetReceipts = (pagePram, callBack) => {
		dispatch(getReceipts({ values: getValues(), pageAndSize: { page: pagePram || page, size } })).then(res => {
			unstable_batchedUpdates(() => {
				callBack && callBack(res.payload);
				setModifiedReceiptData(res.payload?.receipt_vouchers || []);
				setTotalAmount(getTotalAmount(res.payload?.receipt_vouchers || [], 'credit_amount'));
	...
	//get all receipt without pagination
	const handleGetAllReceipts = (callBack, callBackAfterStateUpdated) => {
		dispatch(getAllReceipts(getValues())).then(res => {
			unstable_batchedUpdates(() => {
				callBack && callBack(res.payload);
				setModifiedReceiptData(res.payload?.receipt_vouchers || []);
				setTotalAmount(getTotalAmount(res.payload?.receipt_vouchers || [], 'credit_amount'));
				...
				//get pagination data
				const { totalPages, totalElements } = getPaginationData(res.payload?.receipt_vouchers, size, page);
				setPage(page || 1);
				setSize(size || 25);
		...
		<>
			...
			<table id="table-to-xls" className="w-full" style={{ minHeight: '270px' }}>
				<div ref={componentRef} id="downloadPage">
					...
					{modifiedReceiptData.map((receipt, idx) => {
						return (
							<SinglePage
								classes={classes}
								generalData={generalData}
								reporTitle="Receipt Report"
								...
								serialNumber={receipt.page * receipt.size - receipt.size + 1}
								setPage={setPage}
