...
import { setMakeAListsSearchText } from '../store/makeAListsSlice';
...
const MakeAListsHeader = () => {
	...
	useEffect(() => {
		const alert = localStorage.getItem('makeAListAlert');

		if (alert === 'saveMakeAList') {
			setAlertOpen(true);
			setAlertMessage('Add Success...');
			localStorage.removeItem('makeAListAlert');
		}
		if (alert === 'updateMakeAList') {
			setAlertOpen(true);
			setAlertMessage('Update Success...');
			localStorage.removeItem('makeAListAlert');
		}
		if (alert === 'deleteMakeAList') {
			setAlertOpen(true);
	...
	useEffect(() => {
		const goToFormPage = e => {
			e.key === 'Enter' &&
				...
				setTimeout(() => history.push('/apps/makeAList-management/new'), 0);
		};
	...
	useEffect(() => {
		return () => {
			 ...
			 */
			dispatch(setMakeAListsSearchText({ target: { value: '' } }));
		};
	}, [dispatch]);
	...
	return (
		<div className="flex flex-1 w-full items-center justify-between">
				...
				<ThemeProvider theme={mainTheme}>
					...
					>
						...
						<Input
							...
							onKeyDown={ev => {
								if (ev.key === 'Enter') {
									dispatch(setMakeAListsSearchText(ev));
								}
							}}
						/>
					</Paper>
			...
			<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
				<Button
					component={Link}
					to="/apps/makeAList-management/new"
					className="whitespace-nowrap"
					variant="contained"
					color="secondary"
				>
					<span className="hidden sm:flex">Add New MakeAList</span>
