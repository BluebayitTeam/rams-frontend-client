...
const tableColumns = [
	{ label: 'Sl_No', sortAction: false, serialNo: true },
	{ label: 'Name', name: 'username' },
	{ label: 'Group', name: 'group', subName: 'name' },
	{ label: 'District', name: 'city', subName: 'name' },
	{ label: 'Mobile', name: 'primary_phone' },
...
const PassengerReportsTable = props => {
	...
	const handleGetAllAgents = callBack => {
		dispatch(getAllAgents(getValues())).then(res => {
			...
			//set pagination data
			const { page, size, totalPages, totalElements } = getPaginationData(res.payload?.agents, 5);
			setPage(page);
			setSize(size || 5);
