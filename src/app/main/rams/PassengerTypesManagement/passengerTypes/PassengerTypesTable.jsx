...
import { selectFilteredPassengerTypes, useGetPassengerTypesQuery } from '../PassengerTypesApi';
...
function PassengerTypesTable(props) {
	...
	const { data, isLoading, refetch } = useGetPassengerTypesQuery({ ...pageAndSize, searchKey });
	const totalData = useSelector(selectFilteredPassengerTypes(data));
	const passengerTypes = useSelector(selectFilteredPassengerTypes(data?.passenger_types));
	let serialNumber = 1;
		...
		<div className="w-full flex flex-col min-h-full px-10">
			<FuseScrollbars className="grow overflow-x-auto">
				...
				>
					...
					<TableBody>
						{_.orderBy(passengerTypes, [tableOrder.id], [tableOrder.direction]).map((n) => {
							...
							return (
								...
								>
									...
									>
										<Edit
											onClick={(event) => handleUpdatePassengerType(n, 'updatePassengerType')}
											className="cursor-pointer custom-edit-icon-style"
										/>

										<Delete
											onClick={(event) => handleDeletePassengerType(n, 'deletePassengerType')}
