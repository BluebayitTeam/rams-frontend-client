...
import { removeQualification, saveQualification, updateQualification } from '../store/qualificationSlice';

const NewQualificationHeader = () => {
    const dispatch = useDispatch();
    const methods = useFormContext();
    const { formState, watch, getValues } = methods;
    const { isValid, dirtyFields } = formState;
    const name = watch('degree_name');
    ...
    return (
        <div className="flex flex-1 w-full items-center justify-between">
            <div className="flex flex-col items-start max-w-full min-w-0">
                <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1, transition: { delay: 0.3 } }}>
                    <Typography
                        ...
                        component={Link}
                        role="button"
                        to="/apps/qualification-management/qualifications"
                        color="inherit"
                    >
                        <Icon className="text-20">{theme.direction === 'ltr' ? 'arrow_back' : 'arrow_forward'}</Icon>
                        <span className="hidden sm:flex mx-4 font-medium">Qualifications</span>
                    </Typography>
                </motion.div>
            ...
            >
                ...
                {handleDelete === 'Delete' && routeParams.qualificationId !== "new" && <Button
                ...
                {routeParams.qualificationId === "new" && <Button
                    className="whitespace-nowrap mx-4"
                    variant="contained"
                    color="secondary"
                ...
                {handleDelete !== 'Delete' && routeParams?.qualificationName && <Button
                    className="whitespace-nowrap mx-4"
                    color="secondary"
                    variant="contained"
...
export default NewQualificationHeader;
