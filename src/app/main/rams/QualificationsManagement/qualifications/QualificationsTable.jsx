...
function QualificationsTable(props) {
	...
	const { data, isLoading, refetch } = useGetQualificationsQuery({ ...pageAndSize, searchKey });
	...
	function handleClick(item) {
		navigate(`/apps/qualification/qualifications/${item.id}/${item.handle}`);
	}

	function handleUpdateQualification(item, event) {
		localStorage.removeItem('deleteQualification');
		localStorage.setItem('updateQualification', event);
		navigate(`/apps/qualification/qualifications/${item.id}/${item.handle}`);
	}

	function handleDeleteQualification(item, event) {
		localStorage.removeItem('updateQualification');
		localStorage.setItem('deleteQualification', event);
		navigate(`/apps/qualification/qualifications/${item.id}/${item.handle}`);
	}
		...
		<div className="w-full flex flex-col min-h-full px-10">
			<FuseScrollbars className="grow overflow-x-auto">
				...
				>
					...
					<TableBody>
						{_.orderBy(qualifications, [tableOrder.id], [tableOrder.direction])
