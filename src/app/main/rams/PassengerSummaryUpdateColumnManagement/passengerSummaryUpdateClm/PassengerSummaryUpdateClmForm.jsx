...
function PassengerSummaryUpdateClmForm(props) {
	...
	useEffect(() => {
		...
		dispatch(getPassengerSummaryUpdateClms());
	}, []);
	...
	}, [passengersummaryupdateclmId, watch('image')]);
	...
	const handleCheckPhone = async () => {
		...
		try {
			const response = await axios.get(
				`${CHECK_PRIMARY_PHONE}?primary_phone=${formattedPhoneNumber}&id=${passengersummaryupdateclmId === 'new' ? '' : passengersummaryupdateclmId}&type=${passengersummaryupdateclmId === 'new' ? 'create' : 'update'}`
			);
			...
			<Controller
				...
				render={({ field }) => (
					<TextField
						...
						label={field.value ? 'PassengerSummaryUpdateClm Id' : <span style={{ color: 'red' }}>PassengerSummaryUpdateClm Id</span>}
						id="emp_id_no"
						variant="outlined"
						fullWidth
				...
				render={({ field: { onChange, value } }) => (
					<Autocomplete
						...
						renderInput={(params) => (
							<TextField
								{...params}
								placeholder="Select a passengersummaryupdateclm role"
								label="Role"
								variant="outlined"
								InputLabelProps={value ? { shrink: true } : { style: { color: 'red' } }}
							/>
				...
				render={({ field: { onChange, value } }) => (
					<Autocomplete
						...
						renderInput={(params) => (
							<TextField
								{...params}
								placeholder="Select a passengersummaryupdateclm department"
								label="Department"
								variant="outlined"
								InputLabelProps={value ? { shrink: true } : { style: { color: 'red' } }}
							/>
