...
const schema = yup.object().shape({
	...
	submission_date: yup.string().required('submission_date is required')
});

const VisaCancelList = () => {
	const dispatch = useDispatch();
	// const visaCancelList = useSelector(({ visaCancelListsManagement }) => visaCancelListsManagement.visaCancelList);
	const passengers = useSelector(state => state.data.passengers);

	const [noVisaCancelList, setNoVisaCancelList] = useState(false);
	...
	useEffect(() => {
		return () => {
			/**
			 * Reset VisaCancelList on component unload
			 */
			dispatch(resetVisaCancelList());
			setNoVisaCancelList(false);
		};
	}, [dispatch]);
		...
		return (
			...
			>
				<Typography color="textSecondary" variant="h5">
					There is no such visaCancelList!
				</Typography>
				<Button
					className="mt-24"
					component={Link}
	...
	return (
		<FormProvider {...methods}>
			<FusePageCarded
				...
				header={<NewVisaCancelListHeader />}
				contentToolbar={
					<Tabs
						// value={tabValue}
								...
								render={({ field: { onChange, value, name } }) => (
									<Autocomplete
										...
										onChange={(event, newValue) => {
											if (newValue?.id) {
												axios
													.get(`${VISACANCELLIST_BY_PASSENGER_ID}${newValue?.id}`)
													.then(res => {
														...
														if (res.data.id) {
															...
															history.push(
																`/apps/visaCancelList-management/visaCancelList/${
...
export default withReducer('visaCancelListsManagement', reducer)(VisaCancelList);
