...
import { CREATE_RECRUITINGAGENCY, DELETE_RECRUITINGAGENCY, GET_RECRUITINGAGENCY_BY_ID, UPDATE_RECRUITINGAGENCY } from '../../../../constant/constants';


export const getRecruitingAgency = createAsyncThunk('recruitingAgencyManagement/recruitingAgency/getRecruitingAgency', async (params, { rejectWithValue }) => {
    const authTOKEN = {
        headers: {
...
export const updateRecruitingAgency = createAsyncThunk(
    ...
    async (recruitingAgencyData, { dispatch, getState }) => {
        ...
        const response = await axios.put(`${UPDATE_RECRUITINGAGENCY}${recruitingAgency.id}`, recruitingAgencyData, authTOKEN);
        return response
    }
...
const recruitingAgencySlice = createSlice({
    ...
    extraReducers: {
        ...
        [updateRecruitingAgency.fulfilled]: (state, action) => action.payloHea
    }
})
