...
import { useGetMedicalSaudiReportsQuery } from '../MedicalSaudiReportsApi';
...
const initialTableColumnsState = [
  ...
  {
    id: 5,
    label: 'Medical Exired Date',
    getterMethod: (data) =>
      `${
        data?.medical_expiry_date
          ? moment(new Date(data.medical_expiry_date)).format('DD-MM-YYYY')
          : ''
      } `,
    show: true,
  },
...
function MedicalSaudiReportsTable(props) {
  ...
  const { data: paginatedData } = useGetMedicalSaudiReportsQuery({
  ...
  useEffect(() => {
    if (!inShowAllMode && paginatedData) {
      setModifiedFlightFlightDoneData(paginatedData?.medicals || []);
        ...
        <tbody ref={componentRef} id='downloadPage'>
          {modifiedFlightFlightDoneData.map((flightFlightDone, index) => (
            <SinglePage
              key={index}
              classes={classes}
              reportTitle='Medical Done Report'
...
export default MedicalSaudiReportsTable;
