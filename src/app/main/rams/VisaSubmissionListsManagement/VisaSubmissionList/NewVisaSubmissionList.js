...
import { VISASUBMISSIONLIST_BY_PASSENGER_ID } from 'app/constant/constants.js';
...
const VisaSubmissionList = () => {
    ...
    // const visaSubmissionList = useSelector(({ visaSubmissionListsManagement }) => visaSubmissionListsManagement.visaSubmissionList);
                                ...
                                render={({ field: { onChange, value, name } }) => (
                                    <Autocomplete
                                        ...
                                        onChange={(event, newValue) => {
                                            if (newValue?.id) {
                                                axios.get(`${VISASUBMISSIONLIST_BY_PASSENGER_ID}${newValue?.id}`).then(res => {
                                                    console.log("Res", res.data)
                                                    if (res.data.id) {
                                                        reset({ ...setIdIfValueIsObject(res.data), passenger: newValue?.id })
                                                        history.push(`/apps/visaSubmissionList-management/visaSubmissionList/${newValue?.passenger_id || newValue?.id}`)
                                                    } else {
