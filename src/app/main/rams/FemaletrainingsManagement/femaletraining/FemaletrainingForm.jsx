...
import { useGetFemaletrainingQuery } from '../FemaletrainingsApi';
...
function FemaletrainingForm(props) {
	const methods = useFormContext();
	const { watch } = methods;
	const [generalData, setGeneralData] = useState({});
	const routeParams = useParams();
	const [femaletrainingId, setFemaletrainingId] = useState('');
	...
	const { data: femaletraining } = useGetFemaletrainingQuery(femaletrainingId, {
		skip: !femaletrainingId
	});
	const Comment = '';
	...
	useEffect(() => {
		if (_.isEmpty(femaletraining)) {
			setShowPrint(false);
		} else {
			setShowPrint(true);
		}

		if (routeParams.femaletrainingId !== 'femaletraining-form') {
			setFemaletrainingId(routeParams.femaletrainingId);
		}
	}, [femaletraining, routeParams.femaletrainingId]);
						...
						<div className="md:w-full">
							<div>
								...
								<p>
									...
									<br />
									<br />
									বিষয় : {femaletraining?.target_country?.name || ''} গমনেচ্ছুক কোর্সে ভর্তি প্রসঙ্গে
									।
									<br />
									<br />
									জনাব,
									...
									{generalData?.rl_no}) হতে {femaletraining?.target_country?.name || ''} গমনেচ্ছুক
									...
									<tbody>
										<tr>
											...
											<td style={{ border: '1px solid black' }}>
												{femaletraining?.passenger_name || ''}{' '}
											</td>
											<td style={{ border: '1px solid black' }}>
												{femaletraining?.father_name || ''}{' '}
											</td>
											<td style={{ border: '1px solid black' }}>
												{femaletraining?.passport_no || ''}
											</td>
											<td style={{ border: '1px solid black' }}>
												{femaletraining?.target_country?.name || ''}
											</td>
...
export default FemaletrainingForm;
