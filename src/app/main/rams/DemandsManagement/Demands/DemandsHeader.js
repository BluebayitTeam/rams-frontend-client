...
import { setDemandsSearchText } from '../store/demandsSlice';
...
const DemandsHeader = () => {
    ...
    useEffect(() => {
        const alert = localStorage.getItem("demandAlert")

        if (alert === "saveDemand") {
            setAlertOpen(true)
            setAlertMessage("Add Success...")
            localStorage.removeItem("demandAlert")
        }
        if (alert === "updateDemand") {
            setAlertOpen(true)
            setAlertMessage("Update Success...")
            localStorage.removeItem("demandAlert")
        }
        if (alert === "deleteDemand") {
            setAlertOpen(true)
    ...
    return (
        <div className="flex flex-1 w-full items-center justify-between">
                ...
                <ThemeProvider theme={mainTheme}>
                    ...
                    >
                        ...
                        <Input
                            ...
                            //onBlur={ev => dispatch(setDemandsSearchText(ev))}
                            onKeyDown={(ev) => {
                                if (ev.key === 'Enter') {
                                    dispatch(setDemandsSearchText(ev))
                                }
                            }}
                        />
                    </Paper>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
                <Button
                    component={Link}
                    to="/apps/demand-management/new"
                    className="whitespace-nowrap"
                    variant="contained"
                    color="secondary"
                >
                    <span className="hidden sm:flex">Add New Demand</span>
