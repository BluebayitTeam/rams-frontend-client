...
function PayrollVouchersTable(props) {
  ...
  function handleSelectAllClick(event) {
    if (event.target.checked) {
      setSelected(payrollVouchers?.map((n) => n.id));
      return;
    }
  ...
  function handleClick(item) {
    navigate(`/apps/payrollVoucher/payrollVouchers/${item.id}/${item.handle}`);
  }

  function handleUpdatePayrollVoucher(item, event) {
    localStorage.removeItem('deletePayrollVoucher');
    localStorage.setItem('updatePayrollVoucher', event);
    navigate(`/apps/payrollVoucher/payrollVouchers/${item.id}/${item.handle}`);
  }

  function handleDeletePayrollVoucher(item, event) {
    localStorage.removeItem('updatePayrollVoucher');
    localStorage.setItem('deletePayrollVoucher', event);
    navigate(`/apps/payrollVoucher/payrollVouchers/${item.id}/${item.handle}`);
  }
      ...
      <FuseScrollbars className='grow overflow-x-auto'>
        <TableContainer
          ...
          <Table stickyHeader className='min-w-xl' aria-labelledby='tableTitle'>
            <PayrollVouchersTableHead
              selectedPayrollVoucherIds={selected}
              tableOrder={tableOrder}
              onSelectAllClick={handleSelectAllClick}
              onRequestSort={handleRequestSort}
              rowCount={payrollVouchers?.length}
...
export default withRouter(PayrollVouchersTable);
