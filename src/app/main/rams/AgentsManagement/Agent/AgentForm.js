...
import { getCities, getCountries, getGroups, getThanas, getThanasBasedOnCity } from '../../../../store/dataSlice';
	...
	return (
		<div>
					...
					render={({ field: { onChange, value } }) => (
						<Autocomplete
							...
							renderOption={(pro, option) => {
								console.log(pro);
								return (
									<Box component="li" sx={{ '& > img': { mr: 2, flexShrink: 0 } }} {...pro}>
										<img
											loading="lazy"
											width="20"
			...
			<Controller
				name="date_of_birth"
				control={control}
				render={({ field }) => <CustomDatePicker field={field} label="Birthday" />}
			/>
				...
				render={({ field }) => (
					<FormControl>
						<FormControlLabel
							required
							label="Is active"
							control={<Checkbox {...field} color="primary" checked={field.value || false} />}
						/>
					</FormControl>
				)}
			/>
