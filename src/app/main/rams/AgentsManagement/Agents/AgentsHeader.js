...
import { setAgentsSearchText } from '../store/agentsSlice';
...
const AgentsHeader = () => {
    ...
    useEffect(() => {
        const alert = localStorage.getItem("agentAlert")

        if (alert === "saveAgent") {
            setAlertOpen(true)
            setAlertMessage("Add Success...")
            localStorage.removeItem("agentAlert")
        }
        if (alert === "updateAgent") {
            setAlertOpen(true)
            setAlertMessage("Update Success...")
            localStorage.removeItem("agentAlert")
        }
        if (alert === "deleteAgent") {
            setAlertOpen(true)
    ...
    return (
        <div className="flex flex-1 w-full items-center justify-between">
                ...
                <ThemeProvider theme={mainTheme}>
                    ...
                    >
                        ...
                        <Input
                            ...
                            //onBlur={ev => dispatch(setAgentsSearchText(ev))}
                            onKeyDown={(ev) => {
                                if (ev.key === 'Enter') {
                                    dispatch(setAgentsSearchText(ev))
                                }
                            }}
                        />
                    </Paper>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
                <Button
                    component={Link}
                    to="/apps/agent-management/new"
                    className="whitespace-nowrap"
                    variant="contained"
                    color="secondary"
                >
                    <span className="hidden sm:flex">Add New Agent</span>
