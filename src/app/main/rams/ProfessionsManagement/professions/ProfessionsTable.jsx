...
import { selectFilteredProfessions, useGetProfessionsQuery } from '../ProfessionsApi';

/**
 * The professions table.
 */
function ProfessionsTable(props) {
	const dispatch = useDispatch();
	const { navigate, searchKey } = props;
	const [page, setPage] = useState(0);
	const [rowsPerPage, setRowsPerPage] = useState(50);
	const [pageAndSize, setPageAndSize] = useState({ page: 1, size: 25 });
	const { data, isLoading, refetch } = useGetProfessionsQuery({ ...pageAndSize, searchKey });
	const totalData = useSelector(selectFilteredProfessions(data));
	const professions = useSelector(selectFilteredProfessions(data?.professions));
	let serialNumber = 1;
		...
		<div className="w-full flex flex-col min-h-full px-10">
			<FuseScrollbars className="grow overflow-x-auto">
				...
				>
					...
					<TableBody>
						{_.orderBy(professions, [tableOrder.id], [tableOrder.direction]).map((n) => {
							...
							return (
								...
								>
									...
									>
										<Edit
											onClick={(event) => handleUpdateProfession(n, 'updateProfession')}
											className="cursor-pointer custom-edit-icon-style"
										/>

										<Delete
											onClick={(event) => handleDeleteProfession(n, 'deleteProfession')}
