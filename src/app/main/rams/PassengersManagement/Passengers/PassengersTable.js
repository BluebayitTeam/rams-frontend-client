...
const PassengersTable = props => {
	...
	useEffect(() => {
		passengers = [];
		dispatch(getPassengers({ ...pageAndSize, passengerType: routeParams.passengerType })).then(() =>
			setLoading(false)
		);
	...
	function handleUpdatePassenger(item) {
		...
		props.history.push(
			`/apps/passenger-management/passenger/${item.id}/${item.passenger_id}/${routeParams.passengerType}`
		);
	}
	...
	function handleChangeRowsPerPage(passengerEvent) {
		...
		dispatch(
			getPassengers({
				...pageAndSize,
				size: passengerEvent.target.value,
				passengerType: routeParams.passengerType
			})
		);
	}
						...
						{_.orderBy(
							...
							return (
								...
								>
									...
									<TableCell className="p-4 md:p-16" component="th" scope="row">
										{n.date_of_birth
											? differenceInYears(new Date(), new Date(n.date_of_birth))
											: ''}
									</TableCell>
									...
									<TableCell className="p-4 md:p-16" align="center" component="th" scope="row">
										<div>
											...
											/>{' '}
											<DeleteIcon
												onClick={event => handleDeletePassenger(n, 'Delete')}
												className="cursor-pointer"
												style={{ color: 'red' }}
											/>
										</div>
