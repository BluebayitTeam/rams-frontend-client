...
const PassengersHeader = () => {
	...
	useEffect(() => {
		...
		if (alert === 'savePassenger') {
			setAlertOpen(true);
			setAlertMessage('Add Success...');
			localStorage.removeItem('passengerAlert');
		}
		if (alert === 'updatePassenger') {
			setAlertOpen(true);
			setAlertMessage('Update Success...');
			localStorage.removeItem('passengerAlert');
		}
		if (alert === 'deletePassenger') {
			setAlertOpen(true);
		...
		<div className="flex flex-1 w-full items-center justify-between">
				...
				<ThemeProvider theme={mainTheme}>
					...
					>
						...
						<Input
							...
							onKeyDown={ev => {
								if (ev.key === 'Enter') {
									dispatch(setPassengersSearchText(ev));
								}
							}}
						/>
					</Paper>
			...
			<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
				<Button
					...
					color="secondary"
				>
					<span className="hidden sm:flex">Add New Passenger</span>
