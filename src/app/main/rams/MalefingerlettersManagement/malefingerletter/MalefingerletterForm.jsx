...
import { useGetMalefingerletterQuery } from '../MalefingerlettersApi';
...
function MalefingerletterForm(props) {
	const methods = useFormContext();
	const { watch } = methods;
	const [generalData, setGeneralData] = useState({});
	const routeParams = useParams();
	const [malefingerletterId, setMalefingerletterId] = useState('');
	...
	const { data: malefingerletter } = useGetMalefingerletterQuery(malefingerletterId, {
		skip: !malefingerletterId
	});
	...
	useEffect(() => {
		if (_.isEmpty(malefingerletter)) {
			setShowPrint(false);
		} else {
			setShowPrint(true);
		}

		if (routeParams.malefingerletterId !== 'malefingerletter-form') {
			setMalefingerletterId(routeParams.malefingerletterId);
		}
	}, [malefingerletter, routeParams.malefingerletterId]);
						...
						<div className="md:w-full">
							<div>
								...
								<p>
									...
									<br />
									<br />
									বিষয় : {malefingerletter?.target_country?.name || ''} গমনেচ্ছুক কোর্সে ভর্তি
									প্রসঙ্গে ।
									<br />
									<br />
									জনাব,
									...
									{generalData?.rl_no}) হতে {malefingerletter?.target_country?.name || ''} গমনেচ্ছুক
									...
									<tbody>
										<tr>
											...
											<td style={{ border: '1px solid black' }}>
												{malefingerletter?.passenger_name || ''}{' '}
											</td>
											<td style={{ border: '1px solid black' }}>
												{malefingerletter?.father_name || ''}{' '}
											</td>
											<td style={{ border: '1px solid black' }}>
												{malefingerletter?.passport_no || ''}
											</td>
											<td style={{ border: '1px solid black' }}>
												{malefingerletter?.target_country?.name || ''}
											</td>
...
export default MalefingerletterForm;
