...
import { getLedgers, removeLedgers } from '../store/ledgersSlice';
...
const LedgersTableHead = props => {
	const { selectedLedgerIds } = props;
		...
		<TableHead>
			<TableRow className="h-48 sm:h-64">
				<TableCell padding="none" className="w-40 md:w-64 text-center z-99">
					{props.rowCount !== 0 && !_.isEmpty(selectedLedgerIds) ? (
						<div className="pt-2">
							<IconButton
								className="p-0"
								aria-owns={selectedLedgersMenu ? 'selectedLedgersMenu' : null}
								aria-haspopup="true"
							...
							>
								<MenuList>
									<MenuItem
										onClick={() => {
											dispatch(removeLedgers(selectedLedgerIds)).then(res => {
												res.payload && dispatch(getLedgers(props.pagination));
											});
