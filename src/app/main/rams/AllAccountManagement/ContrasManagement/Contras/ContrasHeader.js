...
import { setContrasSearchText } from '../store/contrasSlice';
...
const ContrasHeader = () => {
	...
	useEffect(() => {
		const alert = localStorage.getItem('contraAlert');

		if (alert === 'saveContra') {
			setAlertOpen(true);
			setAlertMessage('Add Success...');
			localStorage.removeItem('contraAlert');
		}
		if (alert === 'updateContra') {
			setAlertOpen(true);
			setAlertMessage('Update Success...');
			localStorage.removeItem('contraAlert');
		}
		if (alert === 'deleteContra') {
			setAlertOpen(true);
	...
	useEffect(() => {
		const goToFormPage = e => {
			e.key === 'Enter' &&
				...
				setTimeout(() => history.push('/apps/contra-management/new'), 0);
		};
	...
	useEffect(() => {
		return () => {
			 ...
			 */
			dispatch(setContrasSearchText({ target: { value: '' } }));
		};
	}, [dispatch]);
	...
	return (
		<div className="flex flex-1 w-full items-center justify-between">
				...
				<ThemeProvider theme={mainTheme}>
					...
					>
						...
						<Input
							...
							onKeyDown={ev => {
								if (ev.key === 'Enter') {
									dispatch(setContrasSearchText(ev));
								}
							}}
						/>
					</Paper>
			...
			<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
				<Button
					component={Link}
					to="/apps/contra-management/new"
					className="whitespace-nowrap"
					variant="contained"
					color="secondary"
				>
					<span className="hidden sm:flex">Add New Contra</span>
