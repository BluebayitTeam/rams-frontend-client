...
import { setJournalsSearchText } from '../store/journalsSlice';
...
const JournalsHeader = () => {
	...
	useEffect(() => {
		const alert = localStorage.getItem('journalAlert');

		if (alert === 'saveJournal') {
			setAlertOpen(true);
			setAlertMessage('Add Success...');
			localStorage.removeItem('journalAlert');
		}
		if (alert === 'updateJournal') {
			setAlertOpen(true);
			setAlertMessage('Update Success...');
			localStorage.removeItem('journalAlert');
		}
		if (alert === 'deleteJournal') {
			setAlertOpen(true);
	...
	useEffect(() => {
		const goToFormPage = e => {
			e.key === 'Enter' &&
				...
				setTimeout(() => history.push('/apps/journal-management/new'), 0);
		};
	...
	useEffect(() => {
		return () => {
			 ...
			 */
			dispatch(setJournalsSearchText({ target: { value: '' } }));
		};
	}, [dispatch]);
	...
	return (
		<div className="flex flex-1 w-full items-center justify-between">
				...
				<ThemeProvider theme={mainTheme}>
					...
					>
						...
						<Input
							...
							onKeyDown={ev => {
								if (ev.key === 'Enter') {
									dispatch(setJournalsSearchText(ev));
								}
							}}
						/>
					</Paper>
			...
			<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0, transition: { delay: 0.2 } }}>
				<Button
					component={Link}
					to="/apps/journal-management/new"
					className="whitespace-nowrap"
					variant="contained"
					color="secondary"
				>
					<span className="hidden sm:flex">Add New Journal</span>
