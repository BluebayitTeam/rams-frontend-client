...
function JournalsTable(props) {
	...
	const { data, isLoading, refetch } = useGetJournalsQuery({
		...pageAndSize,
		searchKey
	});
	...
	function handleSelectAllClick(event) {
		if (event.target.checked) {
			setSelected(journals.map((n) => n.id));
			return;
		}
	...
	function handleClick(item) {
		navigate(`/apps/journal/journals/${item.id}/${item.handle}`);
	}

	function handleUpdateJournal(item, event) {
		localStorage.removeItem('deleteJournal');
		localStorage.setItem('updateJournal', event);
		navigate(`/apps/journal/journals/${item.id}/${item.invoice_no}`);
	}

	function handleDeleteJournal(item, event) {
		localStorage.removeItem('updateJournal');
			...
			<FuseScrollbars className="grow overflow-x-auto">
				<TableContainer
					...
					>
						...
						<TableBody>
							{_.orderBy(journals, [tableOrder.id], [tableOrder.direction]).map((n) => {
