...
import { useDeleteSubLedgersMutation } from '../SubLedgersApi';

/**
 * The subLedgers table head component.
 */
...
function SubLedgersTableHead(props) {
	const { selectedSubLedgerIds, tableOrder, onSelectAllClick, onRequestSort, rowCount, onMenuItemClick } = props;

	console.log('onMenuItemClick', onMenuItemClick);

	console.log('selectedSubLedgerIds', selectedSubLedgerIds);

	const [removeSubLedgers] = useDeleteSubLedgersMutation();
	...
	function handleDeleteMultipleItem() {
		removeSubLedgers(selectedSubLedgerIds).then((data) => {
			Swal.fire({
				position: 'top-center',
				icon: 'success',
						...
						>
							...
							>
								<MenuList>
									<MenuItem
										onClick={() => {
											removeSubLedgers(selectedSubLedgerIds);
											onMenuItemClick();
