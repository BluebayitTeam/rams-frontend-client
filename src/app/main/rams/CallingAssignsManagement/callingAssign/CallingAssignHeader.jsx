...
function CallingAssignHeader() {
	...
	const [saveCallingAssign] = useUpdateCallingAssignMutation();
	const [removeCallingAssign] = useDeleteCallingAssignMutation();
	...
	function handleUpdateCallingAssign() {
		saveCallingAssign(getValues()).then((data) => {
			UpdatedSuccessfully();
	...
	function handleRemoveCallingAssign(dispatch) {
		removeCallingAssign(callingAssignId);
		DeletedSuccessfully();
		navigate('/apps/callingAssign/callingAssigns');
	...
	return (
		<div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-24 sm:py-32 px-24 md:px-32">
			<div className="flex flex-col items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
				...
				>
					<Typography
						...
						component={Link}
						role="button"
						to="/apps/callingAssign/callingAssigns"
						color="inherit"
					>
						<FuseSvgIcon size={20}>
							{theme.direction === 'ltr'
						...
						<span className="flex mx-4 font-medium">CallingAssigns</span>
					</Typography>
				</motion.div>
				...
				<div className="flex items-center max-w-full">
					...
					>
						{images && images.length > 0 && featuredImageId ? (
							...
							<img
								className="w-32 sm:w-48 rounded"
								src="assets/images/apps/ecommerce/callingAssign-image-placeholder.png"
								alt={name}
							/>
						)}
					</motion.div>
					...
					>
						<Typography className="text-16 sm:text-20 truncate font-semibold">
							{name || 'New CallingAssign'}
						</Typography>
						<Typography
							variant="caption"
							className="font-medium"
						>
							CallingAssign Detail
						</Typography>
					</motion.div>
				</div>
			</div>
			...
			>
				{handleDelete === 'deleteCallingAssign' && callingAssignId !== 'new' && (
					<Typography
						className="mt-6"
						variant="subtitle2"
					>
						Do you want to remove this callingAssign?
					</Typography>
				)}
				{handleDelete === 'deleteCallingAssign' && callingAssignId !== 'new' && (
					<Button
						...
						variant="contained"
						color="secondary"
						onClick={handleRemoveCallingAssign}
				...
				{handleDelete !== 'deleteCallingAssign' &&
					handleUpdate === 'updateCallingAssign' &&
					callingAssignId !== 'new' && (
						<Button
							...
							onClick={handleUpdateCallingAssign}
						>
							Update
						</Button>
					)}
