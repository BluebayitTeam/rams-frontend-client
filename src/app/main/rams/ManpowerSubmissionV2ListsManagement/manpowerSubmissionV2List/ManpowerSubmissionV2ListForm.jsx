...
function ManpowerSubmissionV2ListForm({
  ...
  const { agencies, countries, passengers } = useSelector(
    (state) => state.data
  );
  ...
  useEffect(() => {
    ...
    fetch(
      `${GET_FORM_CONTENT_DETAILS_BY_TITLE}${MANPOWER_SUBMISSION_LIST_FOOTER_V2}`,
      authTOKEN
    )
    ...
    fetch(
      `${GET_FORM_CONTENT_DETAILS_BY_TITLE}${MANPOWER_SUBMISSION_LIST_HEADER_V2}`,
      authTOKEN
    )
      .then((response) => response.json())
      .then((data) =>
        sessionStorage.setItem(
          "formContentHeaderData",
          data?.formcontent_detail[0]?.details || ""
        )
      );
  }, []);
...
export default ManpowerSubmissionV2ListForm;
