...
import { DELETE_AUTHORIZE_REQUEST, UPDATE_CONTRA } from 'src/app/constant/constants';
...
function ContrasTable(props) {
	...
	// ======Authorize Status==========
	async function handleUpdateContraStatus(invoice, type) {
		...
		await axios.put(`${UPDATE_CONTRA}`, data, authTOKEN);
		refetch();
	}
									...
									>
										...
										<TableCell className="p-4 md:p-16 whitespace-nowrap border-t-1  border-gray-200"
											...
											<div className="flex flex-nowrap">

												{
													hasPermission('CONTRA_UPDATE') && (
														<Edit
															style={{
																display:
																	n?.update_status === 'null' &&
																		moment(new Date(n?.contra_date)).format('DD-MM-YYYY') !=
																		currentDate &&
															...
															onClick={() =>
																handleUpdateContraStatus(n?.invoice_no, 'update')
															}
												...
												/>{' '}
												{
													hasPermission('CONTRA_DELETE') && (
														<Delete
															...
															onClick={() =>
																handleUpdateContraStatus(n?.invoice_no, 'delete')
															}
												...
												<Delete
													onClick={() =>
														handleDeleteContra(n, 'deleteContra')}
													className="cursor-pointer custom-delete-icon-style"
													style={{
														display:
															n?.update_status === 'delete_approved' ||
