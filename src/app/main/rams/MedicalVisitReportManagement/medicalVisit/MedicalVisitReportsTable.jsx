...
function MedicalFitReportsTable(props) {
  ...
  const { data: paginatedData } = useGetMedicalVisitReportsQuery({
    skip: inShowAllMode,
  });

  const { data: allData } = useGetMedicalVisitAllReportsQuery({
    skip: !inShowAllMode,
  });

  useEffect(() => {
    if (inShowAllMode && allData) {
      setModifiedMedicalFitData(allData.meets || []);
      ...
      setModifiedMedicalFitData(paginatedData?.meets || []);
  ...
  const handleGetMedicalVisit = useCallback(async (newPage) => {
    try {
  ...
  const handleGetAllMedicalVisit = useCallback(async () => {
    try {
    } catch (error) {
      console.error('Error fetching all medicalVisit:', error);
    }
  }, []);
  ...
  return (
    <div className={classes.headContainer}>
      <ReportPaginationAndDownload
        ...
        onFirstPage={() => handleGetMedicalVisit(1)}
        onPreviousPage={() => handleGetMedicalVisit(page - 1)}
        onNextPage={() => handleGetMedicalVisit(page + 1)}
        onLastPage={() => handleGetMedicalVisit(totalPages)}
        handleExelDownload={handleExelDownload}
        handlePrint={handlePrint}
        handleGetData={handleGetMedicalVisit}
        handleGetAllData={handleGetAllMedicalVisit}
        ...
        <tbody ref={componentRef} id='downloadPage'>
          {modifiedMedicalFitData.map((medicalFit, index) => (
            <SinglePage
              key={index}
              classes={classes}
              reportTitle='Medical Visit Report'
