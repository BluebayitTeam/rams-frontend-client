...
import { FLIGHT_BY_PASSENGER_ID } from 'app/constant/constants';
...
const Flight = () => {
	...
	useEffect(() => {
		if (fromSearch) {
			axios
				.get(`${FLIGHT_BY_PASSENGER_ID}${flightId}`)
				.then(res => {
								...
								render={({ field: { onChange, value } }) => (
									<Autocomplete
										...
										onChange={(event, newValue) => {
											if (newValue?.id) {
												axios
													.get(`${FLIGHT_BY_PASSENGER_ID}${newValue?.id}`)
													.then(res => {
														console.log('Res', res.data);
														if (res.data.id) {
															reset({
