...
const bangladeshAllBanks = useSelector(
  ...
  const [checked, setChecked] = useState(
    paymentVoucherId?.currency ? true : false
  );
  ...
  const cheackDbCdEquality = async () => {
    ...
    if (watch('is_foreign_currency')) {
      const ForeignTotalAmount = totalCreditAmount / getValues().currency_rate;
      setValue(`currency_amount`, ForeignTotalAmount ? ForeignTotalAmount : 0);
    }
                          ...
                          const updatedBankInfo = fields?.map((item, idx) => {
                            if (idx === selectedId) {
                              return {
                                ...
                                cheque_no: getValues(
                                  `items.${selectedId}.cheque_no`
                                ),
                                is_cheque: getValues(
                                  `items.${selectedId}.is_cheque`
                                ),
                                inst_no: getValues(
                                  `items.${selectedId}.inst_no`
                                ),
                                bank_name: getValues(
                                  `items.${selectedId}.bank_name`
                                ),
                                ...
                                pdc_note: getValues(
                                  `items.${selectedId}.pdc_note`
                                ),
                                remarks: getValues(
                                  `items.${selectedId}.remarks`
                                ),
                              };
                            }
                            return item;
