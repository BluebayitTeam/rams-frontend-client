...
function OfficeWorkEditHistorysTable({ officeworkEditHistorysData }) {
  ...
  function handleSelectAllClick(event) {
    if (event.target.checked) {
      setSelected(OfficeWorkLogs.map((n) => n.id));
      return;
    }
  ...
  const handleGetPassengerEditHistorys = useCallback(async (newPage) => {
    try {
      ...
      console.error('Error fetching officeworklogs:', error);
    }
  }, []);
  ...
  const handleGetAllAgents = useCallback(async () => {
    try {
      ...
      console.error('Error fetching all officeworklogs:', error);
    }
  }, []);
  ...
  return (
    <div className={classes.headContainer}>
      <div className='w-full flex flex-col'>
        <div className={classes.headContainer}>
          <div className='w-full flex flex-col'>
            {OfficeWorkLogs && OfficeWorkLogs.length > 0 ? (
              <>
                <div className='grow overflow-x-auto overflow-y-auto'>
                  ...
                  <Table
                    ...
                    <OfficeWorkEditHistorysHead
                      selectedAgentIds={selected}
                      tableOrder={tableOrder}
                      onSelectAllClick={handleSelectAllClick}
                      onRequestSort={handleRequestSort}
                      rowCount={OfficeWorkLogs?.length}
...
export default OfficeWorkEditHistorysTable;
