...
import TrainingEditHistorysHead from './TrainingEditHistorysTableHead';
...
function TrainingEditHistorysTable({ trainingEditHistorysData }) {
  const classes = useStyles();
  const methods = useForm();

  console.log('trainingEditHistorysData', trainingEditHistorysData);
  ...
  const TrainingLogs = trainingEditHistorysData?.training_logs || [];

  useEffect(() => {
    if (trainingEditHistorysData) {
      setTotal(trainingEditHistorysData.training_logs || []);
    }
  }, [trainingEditHistorysData, size, page]);
  ...
  function handleSelectAllClick(event) {
    if (event.target.checked) {
      setSelected(TrainingLogs.map((n) => n.id));
      return;
    }
  ...
  const handleGetPassengerEditHistorys = useCallback(async (newPage) => {
    try {
      ...
      console.error('Error fetching traininglogs:', error);
    }
  }, []);
  ...
  const handleGetAllAgents = useCallback(async () => {
    try {
      ...
      console.error('Error fetching all traininglogs:', error);
    }
  }, []);

  if (trainingEditHistorysData?.length === 0) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
              ...
              <>
                <div className='grow overflow-x-auto overflow-y-auto'>
                  ...
                  <Table
                    ...
                    <TrainingEditHistorysHead
                      selectedAgentIds={selected}
                      tableOrder={tableOrder}
                      onSelectAllClick={handleSelectAllClick}
                      onRequestSort={handleRequestSort}
                      rowCount={TrainingLogs?.length}
                ...
                <div id='pagiContainer' className='flex justify-between mb-6'>
                  <Pagination
                    count={trainingEditHistorysData?.total_pages}
                    defaultPage={1}
                    color='primary'
                    showFirstButton
                    showLastButton
                  ...
                  <TablePagination
                    ...
                    count={trainingEditHistorysData?.total_elements || 0}
...
export default TrainingEditHistorysTable;
