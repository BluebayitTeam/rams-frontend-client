...
import EmbassyEditHistorysHead from './EmbassyEditHistorysHead';
...
function EmbassyEditHistorysTable({ embassyEditHistorysData }) {
  ...
  console.log('dataPrint', embassyEditHistorysData);

  const EmbassyLogs = embassyEditHistorysData?.embassy_logs || [];

  useEffect(() => {
    if (embassyEditHistorysData) {
      setTotal(embassyEditHistorysData.embassy_logs || []);
    }
  }, [embassyEditHistorysData, size, page]);
  ...
  function handleSelectAllClick(event) {
    if (event.target.checked) {
      setSelected(EmbassyLogs.map((n) => n.id));
      return;
    }
  ...
  const handleGetPassengerEditHistorys = useCallback(async (newPage) => {
    try {
      ...
      console.error('Error fetching embassylogs:', error);
    }
  }, []);
  ...
  const handleGetAllAgents = useCallback(async () => {
    try {
      ...
      console.error('Error fetching all embassylogs:', error);
    }
  }, []);

  if (embassyEditHistorysData?.length === 0) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
  ...
  console.log('PrintEmbassy', embassyEditHistorysData?.length);
  ...
  return (
    <div className={classes.headContainer}>
      <div className='w-full flex flex-col'>
        <div className={classes.headContainer}>
          <div className='w-full flex flex-col'>
            {EmbassyLogs && EmbassyLogs.length > 0 ? (
              <>
                <div className='grow overflow-x-auto overflow-y-auto'>
                  ...
                  <Table
                    ...
                    <EmbassyEditHistorysHead
                      selectedAgentIds={selected}
                      tableOrder={tableOrder}
                      onSelectAllClick={handleSelectAllClick}
                      onRequestSort={handleRequestSort}
                      rowCount={EmbassyLogs?.length}
                ...
                <div id='pagiContainer' className='flex justify-between mb-6'>
                  <Pagination
                    count={embassyEditHistorysData?.total_pages}
                    defaultPage={1}
                    color='primary'
                    showFirstButton
                    showLastButton
                  ...
                  <TablePagination
                    ...
                    count={embassyEditHistorysData?.total_elements || 0}
...
export default EmbassyEditHistorysTable;
