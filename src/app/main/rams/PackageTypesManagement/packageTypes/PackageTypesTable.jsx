...
import { selectFilteredPackageTypes, useGetPackageTypesQuery } from '../PackageTypesApi';
...
function PackageTypesTable(props) {
	...
	const { data, isLoading, refetch } = useGetPackageTypesQuery({ ...pageAndSize, searchKey });
						...
						{_.orderBy(data?.package_types, [tableOrder.id], [tableOrder.direction]).map((n) => {
							...
							return (
								...
								>
									...
									>
										<BallotIcon
											onClick={() => navigate(`/apps/packageDetail/packageDetails/${n?.id}`)}
