...
function PostDateChequesTable(props) {
  ...
  const { data, isLoading, refetch } = useGetPostDateChequesQuery({
    ...pageAndSize,
    searchKey,
  });
  const totalData = useSelector(selectFilteredPostDateCheques(data));
  const postDateCheques = useSelector(
    selectFilteredPostDateCheques(data?.postdate_cheques)
  );
  let serialNumber = 1;
  ...
  function handleClick(item) {
    navigate(`/apps/postDateCheque/postDateCheques/${item.id}`);
  }
              ...
              {_.orderBy(
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {n.cheque_date &&
                        moment(new Date(n.cheque_date)).format(
                          "DD-MM-YYYY"
                        )}{" "}
                    </TableCell>
                    <TableCell
                    ...
                    >
                      {hasPermission("POST_DATE_CHEQUE_UPDATE") && (
                        <Edit
                          onClick={(event) =>
                            handleUpdatePostDateCheque(
                              n,
                              "updatePostDateCheque"
                            )
                          }
