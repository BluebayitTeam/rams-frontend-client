...
import {
  CHECK_AVAILABLE_DEMAND_FOR_DEMAND_ASSIGN,
  CHECK_AVAILABLE_VISA_FOR_CALLING_ASSIGN,
  CHECK_CALLING_ASSIGN_EXIST_IN_PASSENGER,
  CHECK_DEMAND_ASSIGN_EXIST_IN_PASSENGER,
...
function DemandAssignForm(props) {
  ...
  const handleCheckAvailableDemand = (id, qty) => {
    ...
    fetch(`${CHECK_AVAILABLE_DEMAND_FOR_DEMAND_ASSIGN}${id}`, authTOKEN)
  ...
  function handleSaveMultipleStatusUpdate(id) {
    if (mltPassengerList?.length >= availableDemand) {
      ...
      fetch(
        `${CHECK_DEMAND_ASSIGN_EXIST_IN_PASSENGER}/${id}/${watch('demand')}`
      )
        .then((response) => response.json())
        .then((data) => {
          if (data?.same_demand) {
  ...
  return (
    <div>
      ...
      {watch('demand') && (
        <h6
          className={`pb-10 ps-5 text-${availableDemand > 0 ? 'green' : 'red'}`}>
          {availableDemand > 0
            ? `Available Demand: ${availableDemand}`
