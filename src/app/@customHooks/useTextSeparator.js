...
import increaseYear from '../@helpers/increaseYear';
  ...
  const separateText = () => {
    if (text) {
      ...
      if (NamePatternText) {
        ...
        const NameFirstIndex =
          NamePatternIndex + NamePatternText?.toString().length;
        const NameTextByFirstIndex = text.slice(NameFirstIndex);
        const NameLineBreakIndex = NameTextByFirstIndex.search(/\n/);
        const cleanNameText = removeDirtyCharacter(
          NameTextByFirstIndex?.slice(0, NameLineBreakIndex)
        );
      ...
      if (FathersNamePatternText) {
        ...
        const FathersNameFirstIndex =
          FathersNamePatternIndex + FathersNamePatternText?.toString().length;
        const FathersNameTextByFirstIndex = text.slice(FathersNameFirstIndex);
        const FathersNameLineBreakIndex =
          FathersNameTextByFirstIndex.search(/\n/);
        const cleanFathersNameText = removeDirtyCharacter(
          FathersNameTextByFirstIndex?.slice(0, FathersNameLineBreakIndex)
        );
      ...
      if (MothersNamePatternText) {
        ...
        const MothersNameFirstIndex =
          MothersNamePatternIndex + MothersNamePatternText?.toString().length;
        const MothersNameTextByFirstIndex = text.slice(MothersNameFirstIndex);
        const MothersNameLineBreakIndex =
          MothersNameTextByFirstIndex.search(/\n/);
        const cleanMothersNameText = removeDirtyCharacter(
          MothersNameTextByFirstIndex?.slice(0, MothersNameLineBreakIndex)
        );
      ...
      if (SpousesNamePatternText) {
        ...
        const SpousesNameFirstIndex =
          SpousesNamePatternIndex + SpousesNamePatternText?.toString().length;
        const SpousesNameTextByFirstIndex = text.slice(SpousesNameFirstIndex);
        const SpousesNameLineBreakIndex =
          SpousesNameTextByFirstIndex.search(/\n/);
        const cleanSpousesNameText = removeDirtyCharacter(
          SpousesNameTextByFirstIndex?.slice(0, SpousesNameLineBreakIndex)
        );
        setSpousesNameText(cleanSpousesNameText);

        const maritalStatusTxt =
          cleanSpousesNameText === 'NA' ? 'Single' : 'Married';
      ...
      if (PermanentAddressPatternText) {
        const PermanentAddressPatternIndex = text.search(
          PermanentAddressPattern
        );
        const PermanentAddressFirstIndex =
          PermanentAddressPatternIndex +
          PermanentAddressPatternText?.toString().length;
        const PermanentAddressTextByFirstIndex = text.slice(
          PermanentAddressFirstIndex
        );
        const PermanentAddressLnBrkIndex1 =
          PermanentAddressTextByFirstIndex.indexOf('\n');
        const PermanentAddressLnBrkIndex2 =
          PermanentAddressTextByFirstIndex.indexOf(
            '\n',
            PermanentAddressLnBrkIndex1 + 1
          );
        setPermanentAddressText(
          PermanentAddressTextByFirstIndex?.slice(
            0,
            PermanentAddressLnBrkIndex2
          ).trim()
        );

        const PermanentAddressText = PermanentAddressTextByFirstIndex?.slice(
          0,
          PermanentAddressLnBrkIndex2
        ).trim();
        ...
        const cleanPostOfficeTxt = removeDirtyCharacter(
          Comma > 3
            ? separatedAddress[1].concat(separatedAddress[2])
            : separatedAddress[1]
        );
        setpost_office(cleanPostOfficeTxt);

        const cleanPoliceStationTxt = removeDirtyCharacter(
          separatedAddress[Comma > 3 ? 3 : 2]
        );
        setpolice_station(cleanPoliceStationTxt);

        const cleanDistrictTxt = removeDirtyCharacter(
          separatedAddress[Comma > 3 ? 4 : 3]
        );
      ...
      if (telephoneNoPatternText) {
        ...
        const telephoneNoFirstIndex =
          telephoneNoPatternIndex + telephoneNoPatternText?.toString().length;
        const telephoneNoTextByFirstIndex = text
          .slice(telephoneNoFirstIndex)
          .trim();
        const telephoneNoSpaceIndex = telephoneNoTextByFirstIndex.search(/\s/);
        const cleantelephoneNoText = removeDirtyCharacter(
          telephoneNoTextByFirstIndex?.slice(0, telephoneNoSpaceIndex)
        );
      ...
      if (allTxtBGDIndex >= 0) {
        ...
        const BGDTextByFirsAndtLastIndex = BGDTextByFirstIndex.slice(
          0,
          BGDTextLastIndex
        );
        ...
        const DOBYear =
          DOBMainText.substr(0, 2) > currentYear.toString().substr(0, 2)
            ? `19${DOBMainText.substr(0, 2)}`
            : `20${DOBMainText.substr(0, 2)}`;
        setdateOfBirthText(
          `${DOBYear}-${DOBMainText.substr(2, 2)}-${DOBMainText.substr(4, 2)}`
        );
        ...
        setpassportExpDateText(
          `20${PspExpDMainTxt.substr(0, 2)}-${PspExpDMainTxt.substr(2, 2)}-${PspExpDMainTxt.substr(4, 2)}`
        );

        const passportExpDateTxt = `20${PspExpDMainTxt.substr(0, 2)}-${PspExpDMainTxt.substr(
          2,
          2
        )}-${PspExpDMainTxt.substr(4, 2)}`;
